...
class ConversationAdapterV2(
  ...
  private var shadowChatEnabled: Boolean = false
  private var shadowChatRegularPasswordValue: CharArray? = null
  private var shadowChatHiddenPasswordValue: CharArray? = null
  ...
  override val isShadowChatEnabled: Boolean
    get() = shadowChatEnabled

  override val shadowChatRegularPassword: CharArray?
    get() = shadowChatRegularPasswordValue

  override val shadowChatHiddenPassword: CharArray?
    get() = shadowChatHiddenPasswordValue
  ...
  fun setShadowChatEnabled(enabled: Boolean) {
    if (shadowChatEnabled == enabled) {
      return
    }

    shadowChatEnabled = enabled
    notifyDataSetChanged()
  }

  fun setShadowChatPasswords(regular: CharArray?, hidden: CharArray?) {
    shadowChatRegularPasswordValue?.fill('\u0000')
    shadowChatHiddenPasswordValue?.fill('\u0000')

    shadowChatRegularPasswordValue = regular?.copyOf()
    shadowChatHiddenPasswordValue = hidden?.copyOf()
    notifyDataSetChanged()
  }
